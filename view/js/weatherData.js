$(function() {
  ...
  var weatherData = {
    ...
    listent: function() {
      ...
      //表单筛选
      // weatherData.querybd(seach1, seach2, seach3, pageNum, pageSize,startTime,endTime);
      //数据查询
      $("#seach").on("click", function() {
        // weatherData.fenyenum()
    ...
    querybd: function(
      ...
      $.ajax({
        ...
        success: function(data) {
          if (data.msg === "获取成功") {
            weatherData.fenye(pageSum);
            weatherData.querydata(data.result.rows);
    ...
    querydata: function(data) {
      ...
      for (let i = 0; i < data.length; i++) {
        var dir = data[i].instantaneousWindDirection
          ? data[i].instantaneousWindDirection
          : "";
        var direction = "";
        ...
        lidata +=
          ...
          "</span>" +
          "<span>" +
          (data[i].currentTemp ? data[i].currentTemp : "&nbsp") +
          "</span>" +
          "<span>" +
          (data[i].currentHumidity ? data[i].currentHumidity : "&nbsp") +
          "</span>" +
          "<span>" +
          (data[i].currentAirPressure ? data[i].currentAirPressure : "&nbsp") +
          "</span>" +
          "<span>" +
          (direction ? direction : "&nbsp") +
          "</span>" +
          "<span>" +
          (data[i].instantaneousWindSpeed
            ? data[i].instantaneousWindSpeed
            : "&nbsp") +
          "</span>" +
          "<span>" +
          (data[i].rainHour1 ? data[i].rainHour1 : "&nbsp") +
          "</span>";
      }
